...
class Camera {
  ...
  void initialize() {
    ...
    auto viewport_upper_left =
        center - focus_dist * w - viewport_u / (floating)2. - viewport_v / (floating)2.;
    //  center of the pixel
    pixel00_loc = viewport_upper_left + (floating)0.5 * (pixel_delta_u + pixel_delta_v);
  ...
  color ray_color(const Ray &r, const int depth,
    ...
    return floating(1. - weight) * color(1., 1., 1.) + floating(weight) * color(.5, .7, 1.);
  }
